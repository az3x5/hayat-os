...
const CalendarModule: React.FC = () => {
  ...
  const selectedDayReminders = MOCK_REMINDERS.filter(reminder => 
    isSameDay(reminder.dueDate, selectedDate)
  );
                 ...
                 selectedDayReminders.map(reminder => (
                  <div key={reminder.id} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-brand-200 transition-colors">
                    <div className={`w-5 h-5 rounded border-2 cursor-pointer flex items-center justify-center ${
                      reminder.completed ? 'bg-brand-600 border-brand-600' : 'border-slate-300 hover:border-brand-500'
                    }`}>
