    ...
    <div className="space-y-6 animate-fade-in pb-20">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center justify-between p-5 border border-slate-200 rounded-xl bg-slate-25">
          ...
          <Switch checked={isPrivacyMode} onChange={togglePrivacyMode} />
        </div>
      </div>
    ...
    <div className="space-y-6 animate-fade-in pb-20">
       <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
             {[
               { id: 'light', label: 'Light', icon: Sun, bg: 'bg-white', border: 'border-slate-200' },
               { id: 'dark', label: 'Dark', icon: Moon, bg: 'bg-slate-900', border: 'border-slate-700' },
               { id: 'system', label: 'System', icon: Monitor, bg: 'bg-slate-50', border: 'border-slate-200' }
             ].map((option) => (
               <button 
                  key={option.id}
                  ...
                  className={`
                    ...
                    ${theme.mode === option.id ? 'border-brand-600 ring-2 ring-brand-100' : 'border-slate-200 hover:border-slate-300'}
                    bg-white
                  `}
               >
                  <div className={`w-full h-24 rounded-lg mb-3 border shadow-sm flex items-center justify-center ${option.bg} ${option.border}`}>
                     <option.icon size={24} className={option.id === 'dark' ? 'text-white' : 'text-slate-900'} />
                  </div>
                  <span className="font-semibold block text-slate-900">{option.label}</span>
