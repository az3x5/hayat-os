...
const FinanceModule: React.FC = () => {
   ...
   const [summary, setSummary] = useState<FinanceSummary>({ balance: 0, income: 0, expenses: 0 });
   const [accounts, setAccounts] = useState<Account[]>([]);
   const [transactions, setTransactions] = useState<Transaction[]>([]);
   const [budgets, setBudgets] = useState<Budget[]>([]);
   const [goals, setGoals] = useState<SavingsGoal[]>([]);
   const [bills, setBills] = useState<Bill[]>(MOCK_BILLS); // Still mock for now
   const [investments, setInvestments] = useState<Investment[]>(MOCK_INVESTMENTS); // Still mock for now
   const [loans, setLoans] = useState<Loan[]>(MOCK_LOANS); // Still mock for now
   ...
   useEffect(() => {
      const fetchData = async () => {
         try {
            setIsLoading(true);
            const [sumData, accData, txData, budData, goalData] = await Promise.all([
            ...
            setSummary(sumData);
            setAccounts(accData);
            setTransactions(txData.map((t: any) => ({ ...t, date: new Date(t.date) })));
            setBudgets(budData);
            setGoals(goalData);
            // Others (Bills, Loans, Investments) still mock/empty for now
         } catch (err) {
   ...
   // --- Derived Metrics ---
   const totalBalance = summary.balance; // Now from API
   const monthlyIncome = summary.income; // Now from API
   const monthlyExpenses = summary.expenses; // Now from API
   ...
   const handleAddItem = async () => {
      ...
      if (addModalType === 'account') {
         const balanceVal = parseFloat(newAccountBalance);
         if (!newAccountName || isNaN(balanceVal)) return;

         if (editingId) {
            // Update existing account
            try {
               const updatedAccount = await FinanceService.updateAccount(editingId, {
                  name: newAccountName,
                  type: newAccountType,
                  balance: balanceVal * (newAccountType === 'credit' ? -1 : 1),
            ...
            // Create new account
            try {
               const newAcc = await FinanceService.createAccount({
                  name: newAccountName,
                  type: newAccountType,
                  balance: balanceVal * (newAccountType === 'credit' ? -1 : 1),
                  accountNumber: newAccountNumber ? `**** ${newAccountNumber.slice(-4)}` : undefined
         ...
         if (!newTitle || isNaN(amountNum)) return;
         const amount = newType === 'expense' ? -Math.abs(amountNum) : Math.abs(amountNum);
         ...
         if (editingId) {
            try {
               const updatedTx = await FinanceService.updateTransaction(editingId, txData);
               setTransactions(transactions.map(t => t.id === editingId ? { ...updatedTx, date: new Date(updatedTx.date) } : t));
               // Re-fetch summary
               const updatedSummary = await FinanceService.getSummary();
               setSummary(updatedSummary);
            } catch (err) { console.error('Failed to update transaction', err); }
         } else {
            try {
               const newTx = await FinanceService.createTransaction(txData);
               setTransactions([{ ...newTx, date: new Date(newTx.date) }, ...transactions]);
               setSummary(prev => ({
                  ...prev,
                  balance: newType === 'income' ? prev.balance + amountNum : prev.balance - amountNum,
                  income: newType === 'income' ? prev.income + amountNum : prev.income,
                  expenses: newType === 'expense' ? prev.expenses + amountNum : prev.expenses,
               }));
                     ...
                     {ledgerEntries.map(entry => (
                        <div key={entry.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all relative group">
                           ...
                           <button
                              ...
                              className={`w-full py-3 rounded-xl font-bold text-sm transition-all border shadow-sm ${entry.status === 'paid'
