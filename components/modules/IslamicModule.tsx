...
const IslamicModule: React.FC = () => {
  ...
  const renderPrayersView = () => {
    ...
    // Dynamic prayers for the main card based on Today's date
    const today = new Date();
    const isTodayFriday = today.getDay() === 5;
    
    const todaysPrayers = MOCK_PRAYER_TIMES.filter(p => {
       if (p.name === 'Jumuah') return isTodayFriday;
       if (p.name === 'Dhuhr') return !isTodayFriday;
       return true;
    });
    ...
    const tableColumns = MOCK_PRAYER_TIMES.filter(p => p.name !== 'Jumuah');
           ...
           <div className="lg:col-span-2 bg-gradient-to-br from-emerald-600 to-teal-800 rounded-3xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden flex flex-col justify-between min-h-[280px]">
              ...
              <div className="relative z-10 flex justify-between items-start mb-2">
                 <div>
                    ...
                    <div className="flex items-end gap-3">
                       <h2 className="text-4xl font-bold tracking-tight">{isTodayFriday ? 'Jumuah' : 'Dhuhr'}</h2>
                 ...
                 <div className="text-right hidden sm:block">
                    <div className="text-xl font-bold">{format(new Date(), 'EEEE, MMM d')}</div>
              ...
              <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
                 {todaysPrayers.map((prayer) => {
                    // Logic to highlight next prayer (mocked as Dhuhr/Jumuah for demo)
                    const isNext = prayer.name === (isTodayFriday ? 'Jumuah' : 'Dhuhr');
                    return (
               ...
               <table className="w-full text-left min-w-[800px]">
                  ...
                  <tbody className="divide-y divide-slate-50">
                     {[0,1,2,3,4,5,6].map((dayOffset) => {
                        ...
                        const jumuahTime = MOCK_PRAYER_TIMES.find(p => p.name === 'Jumuah')?.time;
                        ...
                        return (
                           <tr key={dayOffset} className={`hover:bg-slate-50/50 transition-colors ${isRowFriday ? 'bg-emerald-50/20' : ''}`}>
                              ...
                              {tableColumns.map(p => {
                                 ...
                                 if (p.name === 'Dhuhr' && isRowFriday) {
                                    displayTime = jumuahTime || p.time;
                                    isJumuahCell = true;
                                 }
