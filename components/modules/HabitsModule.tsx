...
const HabitsModule: React.FC = () => {
  ...
  const toggleHabitToday = (id: string) => {
    ...
    setHabits(prev => prev.map(h => {
      if (h.id === id) {
        ...
        const existingLogIndex = newHistory.findIndex(l => l.date === todayStr);

        if (isNowCompleted) {
           if (existingLogIndex >= 0) {
              newHistory[existingLogIndex] = { ...newHistory[existingLogIndex], status: 'completed' };
           } else {
              newHistory.push({ date: todayStr, status: 'completed' });
           }
        } else {
           // If unchecking, remove the log for today (reset to pending)
           if (existingLogIndex >= 0) {
              newHistory.splice(existingLogIndex, 1);
           }
        }
        
        // Simple streak logic: if completing, add 1. If uncompleting, sub 1 (min 0).
        // Note: Real streak calculation should traverse history, but this is a sufficient UI heuristic for now.
        let newStreak = h.streak;
        if (isNowCompleted) newStreak += 1;
  ...
  const toggleHistoryLog = (habitId: string, date: Date) => {
    // Don't allow editing future
    if (date > new Date()) return;

    const dateKey = format(date, 'yyyy-MM-dd');
    setHabits(prev => prev.map(h => {
      if (h.id === habitId) {
        const existingLogIndex = h.history.findIndex(l => l.date === dateKey);
        let newHistory = [...h.history];
        let newStatus: 'completed' | 'skipped' | 'missed' | undefined;

        // Cycle: None -> Completed -> Skipped -> Missed -> None
        if (existingLogIndex >= 0) {
           const currentStatus = newHistory[existingLogIndex].status;
           if (currentStatus === 'completed') newStatus = 'skipped';
           else if (currentStatus === 'skipped') newStatus = 'missed';
           else if (currentStatus === 'missed') newStatus = undefined; // Remove
        } else {
           newStatus = 'completed';
        }
        ...
        if (newStatus) {
           if (existingLogIndex >= 0) {
              newHistory[existingLogIndex] = { ...newHistory[existingLogIndex], status: newStatus };
           } else {
              newHistory.push({ date: dateKey, status: newStatus });
           }
        } else {
           if (existingLogIndex >= 0) {
              newHistory.splice(existingLogIndex, 1);
        ...
        // Also update 'completed' boolean if it's today
